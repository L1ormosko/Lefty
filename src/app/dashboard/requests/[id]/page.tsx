...
export default async function AdvertiserInquiryThread({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { isAdvertiser } = await loadOwnInquiry(id, user).catch(() => notFound());
  if (!isAdvertiser && user.role !== "ADMIN") notFound();
  ...
  return (
    <DashboardShell title={t("inquiry.thread")} nav={advertiserNav()} current="/dashboard/requests">
